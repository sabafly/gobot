...
func (mq *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if query := mq.withGuild; query != nil {
		if err := mq.loadGuild(ctx, query, nodes,
			func(n *Member) { n.Edges.Guild = []*Guild{} },
			func(n *Member, e *Guild) { n.Edges.Guild = append(n.Edges.Guild, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withOwner; query != nil {
		if err := mq.loadOwner(ctx, query, nodes,
			func(n *Member) { n.Edges.Owner = []*User{} },
			func(n *Member, e *User) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MemberQuery) loadGuild(ctx context.Context, query *GuildQuery, nodes []*Member, init func(*Member), assign func(*Member, *Guild)) error {
	...
	nids := make(map[snowflake.ID]map[*Member]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(member.GuildTable)
		s.Join(joinT).On(s.C(guild.FieldID), joinT.C(member.GuildPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(member.GuildPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(member.GuildPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "guild" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (mq *MemberQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Member, init func(*Member), assign func(*Member, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(member.OwnerTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(member.OwnerPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(member.OwnerPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(member.OwnerPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
