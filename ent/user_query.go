...
func (uq *UserQuery) QueryWordSuffix() *WordSuffixQuery {
	query := (&WordSuffixClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOwnGuilds; query != nil {
		if err := uq.loadOwnGuilds(ctx, query, nodes,
	...
	if query := uq.withWordSuffix; query != nil {
...
func (uq *UserQuery) loadOwnGuilds(ctx context.Context, query *GuildQuery, nodes []*User, init func(*User), assign func(*User, *Guild)) error {
...
func (uq *UserQuery) loadWordSuffix(ctx context.Context, query *WordSuffixQuery, nodes []*User, init func(*User), assign func(*User, *WordSuffix)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[snowflake.ID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
