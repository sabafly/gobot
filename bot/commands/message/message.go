...
func Command(c *components.Components) *generic.GenericCommand {
	return (&generic.GenericCommand{
			...
			"/message/pin/delete": generic.PCommandHandler{
				...
				CommandHandler: func(c *components.Components, event *events.ApplicationCommandInteractionCreate) generic.Error {
					...
					if !g.QueryMessagePins().Where(messagepin.ChannelID(event.Channel().ID())).ExistX(event) {
						return generic.NewError(errors.ErrorMessage("errors.unavailable.message.pin", event))
					}
		...
		ModalHandlers: map[string]generic.ModalHandler{
			"message:pin_create_modal": func(component *components.Components, event *events.ModalSubmitInteractionCreate) generic.Error {
		...
		EventHandler: func(c *components.Components, e bot.Event) generic.Error {
			...
			case *events.GuildMessageCreate:
				...
				// ピン留めメッセージの処理
				if err := func(e *events.GuildMessageCreate, c *components.Components) generic.Error {
...
func messageSuffixMessageCreateHandler(e *events.GuildMessageCreate, c *components.Components) generic.Error {
